...
export class DynamoDBTable {
  ...
  constructor(scope: Construct, props: IDynamoDBTableProps) {
    ...
    if (props.writeAccessPrincipalArn) {
      const fullRoleName = `${tableName}-write-to-table-role`

      const writeAccessRole = new iam.Role(scope, fullRoleName, {
        roleName: fullRoleName,
        assumedBy: new iam.ArnPrincipal(props.writeAccessPrincipalArn),
      })

      const writeToDynamoStatement = new iam.PolicyStatement({
        actions: ['dynamodb:PutItem', 'dynamodb:BatchWriteItem', 'dynamodb:UpdateItem'],
        resources: [this.table.tableArn],
      })

      writeAccessRole.addToPolicy(writeToDynamoStatement)
    }
  }
}
